...
async def main():
    ...
    with open('summit_M1M3_heartbeat.log', 'a') as fh:
        l_summit_datetime = []
        now_list_datetime = []
        then_list_datetime = []
        ...
        while True:
            ...
            res_datetime = await cl.query('SELECT "private_seqNum", "private_rcvStamp", '
                                          '"private_sndStamp", "private_kafkaStamp" FROM '
                                          '"efd"."autogen"."lsst.sal.MTM1M3.logevent_heartbeat" WHERE time > now() - 2m')
            then_list_datetime.append(datetime.datetime.utcnow().timestamp())
            if len(res_datetime) == 0:
                l_summit_datetime.append((np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan, np.nan))
            else:
